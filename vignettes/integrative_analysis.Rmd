...
dreme_by_binding_renamed <- dreme_by_binding %>% 
  ...
  # TODO: remove extrainfo after universalmotif 1.9.33 hits devel branch
  update_motifs(extrainfo = TRUE)
```
...
First, we isolate the motifs by ID which are not found in entopic sites.
```{r}
...
Next, we use the motif id's from the unique AME results to filter the Dreme
results, and run TomTom on that subset.
```{r}
...
dreme_by_binding_unique %>% 
  view_tomtom_hits(3) %>%
  # TODO: test  if this fixes subscript error
  cowplot::plot_grid(plotlist = ., nrow = 1, labels = "AUTO", byrow = TRUE)
```
```{r, fig.height=4, fig.width=12, eval=F, include=F}
#TODO: add this type of behavior?
edit_motif <- function(m){
  class(m) <- NULL
  m 
}

motif_rc(dreme_by_binding_unique$motif$m14_CAWTTCG)
dreme_by_binding_unique$motif$m14_CAWTTCG

dreme_by_binding_unique %>% 
  dplyr::mutate(motif = I(ifelse(name == "entopic_CAWTTCG", motif_rc(edit_motif(motif)), edit_motif(motif)))) %>% 
  universalmotif::update_motifs() %>% 
  #dplyr::mutate(motif = ifelse(name == "entopic_CAWTTCG", motif_rc(motif), motif)) %>% 
  #update_motifs() %>% 
  view_tomtom_hits(3) %>% 
...
motif is a better match by eye, or because of domain-specific knowledge. We can
swap the best matched motif using `force_best_match()`.
```{r}
...
all_res %>% 
  ...
  cowplot::plot_grid(plotlist = ., labels = "AUTO")
```
Finally, we can make publication quality figures using cowplot.
