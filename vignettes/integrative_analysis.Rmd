...
The ChIP experiemnts were performed under two conditions: ectopic expression of
...
A subset of annotate E93 ChIP summits from the are shipped with the memes package and can be loaded as follows:
```{r, message=F}
...
Survey is a database of known *Drosophila* TF motifs which we will use throught
...
A key consideration when using a database of known transcription factors is whether to include motifs for TFs which are not expressed. On one hand, including all motifs allows maximum discovery of potential regulators, but on the other hand may detect factors which are not relevant under your experimental conditions. Another consideration is for many tools, increasing the number of tested motifs increases the multiple testing penaly, so limiting the motif database to a smaller number of relevant candidates can be a way to improve statistical power. There is no one right answer to this question. For this vignette, I will give an example for how one could go about filtering a database to consider only expressed factors. For other analyses, this question will need to be carefully considered.
...
Choosing how to decide on whether a gene is expressed or not is a non-trivial topic. **DO NOT BLINDLY DO WHAT I DO HERE**. To keep this vignette on topic, I will use an overly simplistic cutoff of FPKM > 5 to define expressed genes. In this example, I will consider any gene that exceeds the cutoff at either timepoint as expressed.
```{r}
...
Using this filtering strategy, we've decreased our multiple-testing penalty by ~`r round((nrow(meme_db_expressed)/nrow(meme_db)) * 100, 1)`%.
...
the `group` parameter in `ame_plot_heatmap()`. This will stratify the y-axis by
...
we can use AME to test for motif enrichment of the denovo discovered motifs
...
ame_denovo_by_binding %>% 
  ame_plot_heatmap(group = binding_type, scale_max = 10)
```
...
Next, we match the denovo motifs with known TF motifs using TomTom. The TomTom
...
For brevity, I'll only show the top 3 hits for each motif.
...
re-rank the `tomtom` nest data column to set a lower ranked match to the top
...
are significantly different from Increasing and Static scores, but Increasing
and Static scores are not significantly different from eachother, supporting the
