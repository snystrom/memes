...
### Motif scanning with AME (by binding)

### Pre-filtering database for expressed transcription factors
```{r}
meme_db_path <- system.file("extdata/flyFactorSurvey_cleaned.meme", package = "dremeR")
```
...
```{r}
data("example_rnaseq", package = "dremeR")
```
...
We've decreased our multiple-testing penalty by about 50%.
```{r}
nrow(meme_db)
...
matching "Aef1" is reported 2 times within the top 5 hits of the entopic sites
...
```{r}
ame_res %>% 
  dplyr::filter(binding_type == "entopic", rank %in% 1:5) %>% 
  head(5) %>% 
...
large numbers of hits. In contrast to the `vignette("core_ame")` example, the
`-log10(adj.pvalue)` visualization in this context will be more informative. The
`ame_compare_heatmap_methods()` function can be used to visualize why.
...
```{r}
dreme_by_binding_unique %<>% 
  force_best_match(c("entopic_CCCAAAM" = "Eip93F_SANGER_10",
                     "entopic_CCSTCTCC" = "ttk-PF_SANGER_5")) 
```


```{r}
dreme_by_binding_unique %>% 
tidyr::separate(name, c("binding_type"), remove = FALSE, extra = "drop") %>% 
  ggplot(aes(best_match_altname, pos_frac/neg_frac)) +
    geom_col(aes(fill = binding_type)) +
    theme_bw() +
...
expressed_genes %>% 
  dplyr::filter(symbol %in% dreme_by_binding_unique$best_match_altname) %>% 
