...
test_that("database works with user-input settings", {
  ...
  expect_equal(
    basename(search_meme_database_path(path_motif_named)),
    c(basename(fb), "motif")
  )

  noNames <- c(fb, universalmotif::create_motif())
  expect_equal(basename(search_meme_database_path(noNames)), c(fb_name, "2"))

  both_named <- c('flyfactor' = fb, "motif" = universalmotif::create_motif())
  expect_equal(
    basename(search_meme_database_path(both_named)),
    c("flyfactor", "motif")
  )
})
...
test_that("dreme-res works as database", {
  # uses name
  expect_equal(
    search_meme_database_path(list("test" = dreme_out)) %>% basename,
    "test"
  )
...
test_that("motif input method dispatch works", {
  ...
  path <- tempfile()
  expect_equal(
    motif_input(dreme_out, path),
    list(metadata = dreme_out, path = path)
  )

  # universalmotif list
  motifList <- purrr::map(1:2, universalmotif::create_motif)
  expect_equal(
    motif_input(motifList, path),
    list(metadata = as_universalmotif_dataframe(motifList), path = path)
  )

  # universalmotif
  motif <- universalmotif::create_motif()
  expect_equal(
    motif_input(motif, path),
    list(metadata = as_universalmotif_dataframe(motif), path = path)
  )
})
