...
test_that("AME Internal functions work", {
  expect_equal(
    prepareAmeFlags("shuffle", "outdir"),
    c("--control", "--shuffle--", "--oc", "outdir")
  )
  expect_equal(
    prepareAmeFlags("test.fa", "outdir"),
    c("--control", "test.fa", "--oc", "outdir")
  )
...
test_that("input error checking works", {
  fa1 <- system.file(
    "extdata/fasta_ex/fa1.fa",
    package = "memes",
    mustWork = TRUE
  )
  fa2 <- system.file(
    "extdata/fasta_ex/fa2.fa",
    package = "memes",
    mustWork = TRUE
  )
  db <- system.file(
    "extdata/flyFactorSurvey_cleaned.meme",
    package = "memes",
    mustWork = TRUE
  )
...
test_that("AME Runs", {
  ...
  expect_s3_class(
    ame1 <- runAme(seq, NULL, evalue_report_threshold = 100, database = db),
    "data.frame"
  )
  ...
  expect_identical(
    runAme(seq, NA, evalue_report_threshold = 100, database = db),
    ame1
  )
  ...
  ame_discriminative <<- runAme(
    seq,
    seq2,
    database = db,
    evalue_report_threshold = 50
  )
...
test_that("List input works", {
  withr::local_options(list(meme_db = NULL))
  seqList <- list("seq" = seq, "seq2" = seq2)

  # use discriminative background from list
  # be sure this matches the runAme call from the ame_discriminative chunk above
  expect_identical(
    runAme(seqList, "seq2", database = db, evalue_report_threshold = 50)$seq,
    ame_discriminative
  )

  seqList2 <- list("A" = prepare_seq(5, 123), "B" = prepare_seq(5, 321))
...
test_that("list input similar runDreme tests", {
  ...
  expect_equal(
    "NO",
    "Need to write test for list input to runAme similar to runDreme tests"
  )
})
