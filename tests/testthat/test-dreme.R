...
test_that("Dreme flags correctly parsed", {
  expect_equal(
    prepareDremeFlags(input = "input.fa", control = "shuffle"),
    c("-p", "input.fa")
  )
  expect_equal(
    prepareDremeFlags(input = "input.fa", control = "background.fa"),
...
test_that("dreme_results & universalmotif list validators work", {
  ...
  expect_false(is_universalmotif_list(list(
...
test_that("runDreme dispatch works", {
  ...
  expect_error(
    runDreme(seq_by_type, c("B", "C"), e = 70),
    "names passed to control do not exist"
  )
  expect_error(
    runDreme(seq_by_type, c("shuffle", "C"), e = 70),
    "names passed to control do not exist"
  )
  expect_error(
    runDreme(seq_by_type, c("shuffle", "B"), e = 70),
    "names passed to control do not exist"
  )

  ## Ensure error catch works with list input also
  expect_error(
    suppressMessages(runDreme(seq_by_type, "A", et = 70)),
    "\"e\" instead of",
    class = "error"
  )
})
...
test_that("runDreme error checking works", {
  ## test error checking
  expect_error(
    suppressMessages(runDreme(fa, "shuffle", et = 39)),
    "\"e\" instead of",
    class = "error"
  )
})
